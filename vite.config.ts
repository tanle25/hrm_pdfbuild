...
export default defineConfig(({mode}) => {
  ...
  return {
    plugins: [
      ...
      tailwindcss(),
      {
        name: 'profile-translation-api',
        configureServer(server) {
          server.middlewares.use('/api/translate-profile', async (req, res) => {
            if (req.method !== 'POST') {
            ...
            if (!geminiApiKey || geminiApiKey === 'MY_GEMINI_API_KEY') {
              sendJson(res, 500, {
                error: 'Missing GEMINI_API_KEY. Add it to .env.local before exporting with AI translation.',
              });
              return;
            }
